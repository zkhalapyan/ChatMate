...
$(document).ready(function(){chatmate.services.ChatServiceInitializer();chatmate.controllers.PageController.register("HomePage",chatmate.init.ChatRooms);chatmate.controllers.PageController.register("NewChatRoomPage",chatmate.init.NewChatRoom);chatmate.controllers.PageController.register("ChatRoomsView",chatmate.init.ChatRooms);chatmate.controllers.PageController.register("ChatRoomPage",chatmate.init.ChatRoomMessages);chatmate.controllers.PageController.register("NewChatRoomPage",chatmate.init.NewChatRoom);
chatmate.models.ChatRoomModelFactory.getChatRoomModel("OPT Tech Talk");chatmate.models.ChatRoomModelFactory.getChatRoomModel("MWF Friends");chatmate.controllers.PageController.openChatRoomsView();if(chatmate.utils.DeviceDetection.isDeviceAndroid()){var $androidBackButtonCallback$$=function $$androidBackButtonCallback$$$(){chatmate.controllers.PageController.canGoBack()?(console.log("Go back!"),chatmate.controllers.PageController.goBack()):navigator.app.exitApp()};document.addEventListener("deviceready",
function(){document.addEventListener("backbutton",$androidBackButtonCallback$$,!0)},!1)}else chatmate.services.ChatService.run()});
...
return $that$$}}();
chatmate.services.ChatServiceInitializer=function $chatmate$services$ChatServiceInitializer$(){var $JSCompiler_temp_const$$=chatmate.services,$that$$={},$logger$$=chatmate.utils.Logger("chatmate.services.ChatService"),$messageQueue$$=[],$socket$$=null,$onMessageCallback$$,$isSocketOpen$$=!1,$onOpenHandler$$=function $$onOpenHandler$$$(){console.log("ChatService: Opened connection.");$socket$$.binaryType="arraybuffer";$isSocketOpen$$=!0},$onMessageHandler$$=function $$onMessageHandler$$$($e$$){var $packet$$=$.parseJSON($e$$.data);
console.log("ChatService: Received message: "+$e$$.data);chatmate.models.ChatRoomModelFactory.getChatRoomModel($packet$$.room).addMessage($packet$$.message);"function"===typeof $onMessageCallback$$&&$onMessageCallback$$($packet$$)},$onCloseHandler$$=function $$onCloseHandler$$$(){$isSocketOpen$$=!1};$that$$.setOnMessageCallback=function $$that$$$setOnMessageCallback$($newOnMessageCallback$$){$onMessageCallback$$=$newOnMessageCallback$$};$that$$.run=function $$that$$$run$(){null===$socket$$&&($logger$$.info("Starting WebSocket."),
...
$packet$$),$socket$$.send(JSON.stringify($packet$$))):$messageQueue$$.push($message$$)};$JSCompiler_temp_const$$.ChatService=$that$$};
...
": ")},warn:function(){$warn$$($namespace$$,arguments)},error:function(){$error$$($namespace$$,arguments)},info:function(){$info$$(arguments)}}}}();chatmate.utils.UUIDGen={generate:function $chatmate$utils$UUIDGen$generate$(){var $s$$=[],$i$$;for($i$$=0;36>$i$$;$i$$+=1)$s$$[$i$$]="0123456789abcdef".substr(Math.floor(16*Math.random()),1);$s$$[14]="4";$s$$[19]="0123456789abcdef".substr($s$$[19]&3|8,1);$s$$[8]=$s$$[13]=$s$$[18]=$s$$[23]="-";return $s$$.join("")}};
